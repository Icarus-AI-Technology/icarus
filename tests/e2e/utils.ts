...
export function getByTestId(pageOrLocator: Page | Locator, id: string) {
  ...
  // @ts-expect-error - Playwright version compatibility check
  if (typeof pageOrLocator.getByTestId === 'function') {
    // @ts-expect-error - Using newer Playwright API if available
    return pageOrLocator.getByTestId(id);
  }
